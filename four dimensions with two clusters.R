...
# covariance matrix
cov = matrix(c(1, 0.5, 0.2, 0.3,
               0.5, 1, 0.4, 0.2,
               0.2, 0.4, 1, 0.5,
               0.3, 0.2, 0.5, 1), nrow = 4)
...
# loop that generates 50 dataframes, introduces missing data 50 times and imputes each data frame with missing data. Warning: This loop is slightly memory intensive and takes a while.
for (i in 1:50) {
  data = as.data.frame(mvrnorm(n = n, mu = mean, Sigma = cov))
  colnames(data) = c("Var1", "Var2", "Var3", "Var4")
...
box = function(data, ylable, title, filename, ylimit = c(0.4, 0.9), color = "bisque2") {
  ...
  ggsave(filename = filename, path = "./4d2c/")
  return(plot)
}

box(jaccardMean, "Jaccard Index", "Boxplot of the Jaccard Indices for mean imputation", "meanJaccard.jpg", c(0.75,1))
box(jaccardPMM, "Jaccard Index", "Boxplot of the Jaccard Indices for imputation using mice", "meanPMM.jpg", c(0.75,1))
box(jaccardRanger, "Jaccard Index", "Boxplot of the Jaccard Indices for imputation using ranger", "meanRanger.jpg", c(0.75,1))


box(randMean, "Rand Index", "Boxplot of the Rand Indices for mean imputation", "randMean.jpg", c(0.75,1), "cadetblue2")
box(randPMM, "Rand Index", "Boxplot of the Rand Indices for imputation using mice", "randMice.jpg",c(0.75,1), "cadetblue2")
box(randRanger, "Rand Index", "Boxplot of the Rand Indices for imputation using ranger", "randRanger.jpg",c(0.75,1), "cadetblue2")
